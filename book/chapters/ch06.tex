...
\end{equation}
the true value of state $s$ equals the expected immediate reward plus the discounted value of the next state.  Equation~\eqref{eq:bellman-v-ch6} is exact when $V = V^\pi$, but even with an imperfect estimate $V \approx V^\pi$ the quantity
\begin{equation}
\label{eq:td0-target}
...
&= V^\pi(s) - V^\pi(s) = 0,
\end{align*}
where the second line uses the Bellman equation~\eqref{eq:bellman-v-ch6}.
\end{proof}
...
\end{equation}
Replacing the expectation in~\eqref{eq:bellman-q-ch6} by a single sample and applying the resulting update gives:
