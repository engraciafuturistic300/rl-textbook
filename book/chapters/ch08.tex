...
Assume $\pi_\theta(a \mid s)$ is twice continuously differentiable in $\theta$, and that the gradient $\nabla_\theta \log \pi_\theta$ is bounded.  If the step sizes satisfy~\eqref{eq:rm-conditions-ch8} and $J(\theta)$ is bounded above, then the iterates of REINFORCE satisfy
\[
...
\item \textbf{GAE (Generalised Advantage Estimation, Section~\ref{sec:gae}) with $\lambda \in [0,1]$:} exponentially weighted average of $n$-step returns, combining low bias ($\lambda$ near 1) with low variance ($\lambda$ near 0).
\end{itemize}
...
Table~\ref{tab:pg-comparison-ch8} summarises the properties of the estimators developed in this chapter.  We use the following notation: $\hat g$ denotes the gradient estimator, $b$ denotes the baseline, and $K$ denotes the number of samples per prompt/state.
...
Several observations are worth drawing from Table~\ref{tab:pg-comparison-ch8}.
...
\begin{enumerate}
\item State the Robbins-Monro conditions~\eqref{eq:rm-conditions-ch8} and give one example of a step-size sequence that satisfies them and one that does not.
