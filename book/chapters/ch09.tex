...
changing the expected gradient (Theorem~\ref{thm:baseline} in Chapter~\ref{ch:pg}).
...
\begin{proof}
By Definition~\ref{def:advantage-ch9} and linearity of expectation,
\[
...
\]
We now proceed more carefully.  For $A < 0$ and $r < 1-\varepsilon$:
...
\begin{enumerate}[label=(\alph*)]
  \item Compute $\hat{A}_0^{\mathrm{GAE}}$ using Definition~\ref{def:gae-ch9}.
