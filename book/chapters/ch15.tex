...
(Section~\ref{sec:rlvr-ch15}). We then examine chain-of-thought\index{chain-of-thought}
...
Standard RLHF uses the KL penalty of~\eqref{eq:rlhf-objective-ch15} to
...
The GRPO algorithm (Chapter~\ref{ch:grpo}) computes advantages by
...
of Section~\ref{sec:rlvr-ch15}, now extended to multi-step trajectories.
...
\item[Code execution] The agent writes code, executes it in a
  ...
  unit-test reward signal from Section~\ref{sec:rlvr-ch15} applies
  directly.
...
Compute the GRPO advantage estimates $\hat{A}_i$ from~\eqref{eq:grpo-advantage-ch15}.
